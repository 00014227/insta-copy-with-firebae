...
const SubscriptionButton = ({ publicationUserID }) => {
  ...
  useEffect(() => {
    ...
    // Cleanup function
    return () => {
      // Cleanup any subscriptions or listeners if necessary
    };
  }, []);
  ...
  return (
    <div>
      {isSubscribed ? (
        <button className=' bg-red-500 text-white text-lg px-3 rounded-md font-semibold' onClick={handleSubscription}>Unsubscribe</button>
      ) : (
        <button className=' bg-blue-500 text-white text-lg px-3 rounded-md font-semibold' onClick={handleSubscription}>Subscribe</button>
      )}
    </div>
  );
};
